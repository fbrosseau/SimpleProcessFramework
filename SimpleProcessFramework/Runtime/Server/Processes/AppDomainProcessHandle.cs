...
namespace Spfx.Runtime.Server.Processes
{
    ...
    internal class AppDomainProcessHandle : GenericChildProcessHandle
    {
        ...
        protected override Task<IProcessSpawnPunchHandles> SpawnProcess()
        {
            ...
            var handles = new GenericProcessSpawnPunchHandles();
            RemotePunchPayload.WritePipe = handles.ReadPipe.GetClientHandleAsString();
            RemotePunchPayload.ReadPipe = handles.WritePipe.GetClientHandleAsString();

            Action callback = new AppDomainStartData
            {
                InputString = RemotePunchPayload.SerializeToString()
            }.CrossDomainCallback;
            ...
            }).ContinueWith(t => OnProcessLost("AppDomain callback failed: " + t.GetFriendlyException()), TaskContinuationOptions.OnlyOnFaulted);

            return Task.FromResult((IProcessSpawnPunchHandles)handles);
        }
    }
#endif
}
